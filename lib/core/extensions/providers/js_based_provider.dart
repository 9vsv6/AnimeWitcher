...
class JsBasedProvider extends SkyStreamProvider {
  ...
  // Legacy JavaScript extensions may not expose the independent detail
  // section functions. Their compatibility fallbacks all call getDetails(),
  // so share a short-lived request/result instead of fetching the same anime
  // up to five times in parallel.
  static const Duration _detailsCompatibilityCacheTtl = Duration(seconds: 15);
  static const int _maxDetailsCompatibilityCacheEntries = 12;
  final Map<String, Future<MultimediaItem>> _detailsRequestCache = {};
  final Map<String, DateTime> _detailsRequestCacheTimes = {};
  ...
  Future<MultimediaItem> getDetails(String url) {
    ...
    final cachedAt = _detailsRequestCacheTimes[url];
    final cachedRequest = _detailsRequestCache[url];
    if (cachedAt != null &&
        cachedRequest != null &&
        now.difference(cachedAt) < _detailsCompatibilityCacheTtl) {
      return cachedRequest;
    }

    _detailsRequestCache.remove(url);
    _detailsRequestCacheTimes.remove(url);

    if (_detailsRequestCache.length >=
        _maxDetailsCompatibilityCacheEntries) {
      String? oldestKey;
      DateTime? oldestTime;
      for (final entry in _detailsRequestCacheTimes.entries) {
      ...
      if (oldestKey != null) {
        _detailsRequestCache.remove(oldestKey);
        _detailsRequestCacheTimes.remove(oldestKey);
      }
    }

    final request = _loadDetailsUncached(url);
    _detailsRequestCache[url] = request;
    _detailsRequestCacheTimes[url] = now;
    unawaited(
      request.then<void>(
        (_) {},
        onError: (Object _, StackTrace __) {
          if (identical(_detailsRequestCache[url], request)) {
            _detailsRequestCache.remove(url);
            _detailsRequestCacheTimes.remove(url);
          }
        },
  ...
  Future<MultimediaItem> _loadDetailsUncached(String url) async {
    await _ensureReady();
    if (_error != null) throw JsPluginException("INIT_ERROR", _error!);
