...
void main() {
  ...
  group('queue gate / overlay', () {
    ...
    test('stale saved progress never upgrades an attached task to running', () {
      expect(activeNativeAttachmentStatus(TaskStatus.enqueued), TaskStatus.enqueued);
      expect(activeNativeAttachmentStatus(TaskStatus.running), TaskStatus.running);
      expect(
        activeNativeAttachmentStatus(TaskStatus.waitingToRetry),
        TaskStatus.waitingToRetry,
      );
      expect(activeNativeAttachmentStatus(TaskStatus.paused), TaskStatus.enqueued);
      expect(activeNativeAttachmentStatus(TaskStatus.failed), TaskStatus.enqueued);
      expect(activeNativeAttachmentStatus(null), TaskStatus.enqueued);
    });

    test('recovery waits for a real running callback after resume starts', () {
      expect(
        recoveredDownloadDisplayStatus(
          persisted: TaskStatus.running,
          userPaused: false,
          queueWaiting: false,
          stillNative: false,
    ...
    test('server-rejected native pause is never persisted as user-paused', () {
      expect(
        shouldCommitUserPause(
          hadLiveNativeTask: true,
