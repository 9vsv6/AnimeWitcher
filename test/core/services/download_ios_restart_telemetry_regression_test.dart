...
void main() {
  ...
  test('native iOS live bytes drive aggregate speed before part finalization', () async {
    final directory = await Directory.systemTemp.createTemp('ios-speed-regression-');
  ...
  test('restored multipart owners that disappear are recovered without callbacks', () async {
    final directory = await Directory.systemTemp.createTemp('ios-restart-regression-');
    final records = <String, TaskRecord>{};
    final parent = ParallelDownloadTask(
    ...
    PersistentParallelDownload createCoordinator() => PersistentParallelDownload(
      startPart: (task, progress, size) async {
        starts.add(task);
        return true;
      },
