...
void main() {
  ...
  test('native 0.999 sentinel never adds phantom bytes across recovery', () async {
    ...
    coordinator.handleUpdate(
      TaskProgressUpdate(current, 0.60, 100, 1, const Duration(seconds: 1)),
    );
    await waitUntil(() => (records[parent.taskId]?.progress ?? 0) >= 0.599);
    ...
    coordinator.handleUpdate(
      TaskProgressUpdate(current, 0.999, 100, 0, const Duration(seconds: -1)),
    );
    ...
    coordinator.handleUpdate(
      TaskProgressUpdate(current, 0.62, 100, 1, const Duration(seconds: 1)),
    );
    await waitUntil(() => (records[parent.taskId]?.progress ?? 0) >= 0.619);
  ...
  test(
    'legacy manifest drops three fake 0.999 ranges from parent progress',
    () async {
      await coordinator.dispose();
      starts.clear();
