...
void main() {
  ...
  test('adopts a full HTTP 200 child without downloading the episode again', () async {
    ...
    try {
      ...
      expect(statuses.where((status) => status == TaskStatus.complete).length, 1);
      expect(starts.length, 1, reason: 'must not start a replacement full GET');
      expect(canceled, hasLength(1));
      expect(
        canceled.single.toSet(),
        {
  ...
  test('does not promote an oversized ranged 206 as a sequential fallback', () async {
    final directory = await Directory.systemTemp.createTemp(
    ...
    try {
      ...
      expect(await child.exists(), isTrue, reason: 'keep bytes for diagnosis/resume');
    } finally {
      await coordinator.dispose();
      if (await directory.exists()) await directory.delete(recursive: true);
    }
  ...
  test('automatic fallback never overwrites an unexpected existing target', () async {
    final directory = await Directory.systemTemp.createTemp(
