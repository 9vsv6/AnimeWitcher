...
Map<String, dynamic> _document(String id) {
  return <String, dynamic>{
    'name': 'projects/animewitcher-1c66d/databases/(default)/documents/anime_list/$id',
          ...
          handler.resolve(
            Response<dynamic>(
              ...
              data: <Map<String, dynamic>>[
                <String, dynamic>{'document': _document('one')},
                <String, dynamic>{'document': _document('two')},
                <String, dynamic>{'document': _document('three')},
              ],
            ),
          );
          return;
...
void main() {
  test('broadcast schedule page requests one bounded Firestore page', () async {
    ...
    final page = await _provider(stub.dio).getBroadcastSchedulePage(
      'السبت',
      limit: 2,
    );
  ...
  test('broadcast schedule page ignores an unknown day without a network call', () async {
    final stub = _stubDio();

    final page = await _provider(stub.dio).getBroadcastSchedulePage('غير معروف');
