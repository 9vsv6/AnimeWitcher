...
void main() {
  group('normalizePageEscapes', () {
    test('leaves plain text untouched', () {
      expect(normalizePageEscapes('https://cdn.example.com/v/a.mp4'),
    ...
    test('decodes JS unicode escapes', () {
      expect(
    ...
    test('decodes numeric and hex HTML entities', () {
      expect(
        normalizePageEscapes('https&#58;&#47;&#47;ex.com/v.mp4'),
        'https://ex.com/v.mp4',
      );
      expect(
        normalizePageEscapes('https&#x3A;&#x2F;&#x2F;ex.com/v.mp4'),
  ...
  group('extractBetweenMarkers', () {
    ...
    test('cuts between the first markers and removes the marker text', () {
      const body = '{"ok":true,"file":"https://cdn.x/hls/master.m3u8",'
          '"q":720,"t":9}';
      expect(extractBetweenMarkers(body, start, end),
          'https://cdn.x/hls/master.m3u8');
    });

    test('returns empty when the end marker is missing', () {
      expect(extractBetweenMarkers('{"file":"https://x/v.mp4"}', start, end),
          '');
    });
    ...
    test('returns empty for empty inputs', () {
      expect(extractBetweenMarkers('', start, end), '');
      expect(extractBetweenMarkers('body', '', end), '');
      expect(extractBetweenMarkers('body', start, ''), '');
    });
  });

  group('extractServerUrlWithRetry', () {
    const start = 'file":"';
    const end = '","q"';
    ...
    test('short-circuits when nothing changed and no markers matched', () {
      const body = 'plain page without markers';
      expect(
        extractServerUrlWithRetry(body: body, start: start, end: end),
        '',
      );
