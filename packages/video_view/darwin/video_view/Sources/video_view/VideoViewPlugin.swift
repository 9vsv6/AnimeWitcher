...
class VideoController: NSObject, FlutterStreamHandler {
	...
	private func openResolved(source: String, headers: [String: String]? = nil) {
		...
		if uri == nil {
			...
			self.source = source
			state = 1
			// Phase 2: inject custom HTTP headers into ALL requests (manifest + every HLS segment)
			// AVURLAssetHTTPHeaderFieldsKey is the industry-standard approach used by Netflix/YouTube
			let asset: AVURLAsset
			if let h = headers, !h.isEmpty, !uri!.isFileURL {
				asset = AVURLAsset(url: uri!, options: ["AVURLAssetHTTPHeaderFieldsKey": h])
			} else {
				asset = AVURLAsset(url: uri!)
			}
			let item = AVPlayerItem(asset: asset)
			item.preferredPeakBitRate = maxBitrate
			item.preferredMaximumResolution = maxWidth == 0 && maxHeight == 0 ? CGSizeZero : CGSize(width: maxWidth, height: maxHeight)
			
			// Native-parity buffering
			item.preferredForwardBufferDuration = 15.0 // Min buffer hint
	...
	private func setPosition(time: CMTime) {
		if time != position {
			...
			if streaming || avPlayer.currentItem?.duration == .indefinite {
				if let range = avPlayer.currentItem?.seekableTimeRanges.last?.timeRangeValue {
					map["duration"] = Int(range.duration.seconds * 1000)
				}
			}
			eventSink?(map)
		}
		...
		case #keyPath(AVPlayer.currentItem.status):
			...
			case .readyToPlay:
				if state == 1 {
					retryCount = 0
					if (streaming) {
						// For live streams, AVPlayer handles seeking to edge naturally
						loadEnd()
